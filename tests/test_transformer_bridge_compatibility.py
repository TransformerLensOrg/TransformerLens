...
class TestTransformerBridgeCompatibility:
    ...
    def test_forward_pass_equivalence(self, models, test_text):
        ...
        assert abs(ht_loss - bridge_loss) < 1e-5, f"Loss mismatch: HT={ht_loss:.6f}, Bridge={bridge_loss:.6f}"
    ...
    def test_logits_equivalence(self, models, test_text):
        ...
        assert torch.allclose(ht_logits, bridge_logits, rtol=1e-4, atol=1e-5), "Logits should be nearly identical"
    ...
    def test_hook_functionality_equivalence(self, models, test_text):
        ...
        assert abs(ht_effect - bridge_effect) < 1e-5, f"Ablation effects should match: HT={ht_effect:.6f}, Bridge={bridge_effect:.6f}"
    ...
    def test_weight_sharing_verification(self, models, test_text):
        ...
        assert abs(ht_change - bridge_change) < 1e-4, "Models should respond similarly to weight changes"
    ...
    def test_component_structure_equivalence(self, models):
        ...
        assert ht.embed.W_E.shape == bridge.embed.W_E.shape, "Embedding weights should have same shape"
        assert ht.blocks[0].attn.W_V.shape == bridge.blocks[0].attn.W_V.shape, "Attention weights should have same shape"
    ...
    def test_no_persistent_hookedtransformer_reference(self, models):
        ...
        assert not hasattr(bridge, '_processed_hooked_transformer'), "Should not have persistent HookedTransformer reference"
