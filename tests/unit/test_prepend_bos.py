    ...
    def test_pass_prepend_bos_at_method_level(self, model):
        for global_prepend_bos in [True, False]:
            ...
            for local_prepend_bos in [True, False]:
                logits = model(self.prompt, prepend_bos=local_prepend_bos)  # [batch pos d_vocab]
                str_tokens = model.to_str_tokens(self.prompt, prepend_bos=local_prepend_bos)
