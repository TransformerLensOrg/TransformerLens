...
class TestLossGateOnTorchBridge:
    ...
    def _build_bridge(self, provides_sequence_logits: bool):
        from tests.mocks.mock_logits_bridge import (
            LogitsDriver,
            build_mock_logits_bridge,
        )

        psl = provides_sequence_logits

        class FinalPositionDriver(LogitsDriver):
            provides_sequence_logits = psl

        return build_mock_logits_bridge(driver_cls=FinalPositionDriver)
