...
def _wire_attention_bridge(
    ...
    return wire_attention_bridge(
        adapter, fake_attn, expected_type=PositionEmbeddingsAttentionBridge
    )
...
class TestOlmoAttentionBridge:
    ...
    def test_forward_executes_and_matches_hook_shapes(
        ...
        position_embeddings = identity_rope(seq_len, cfg.d_head)
...
class TestOlmoClipQkv:
    ...
    def test_clamp_is_out_of_place_under_backward_hooks(
        ...
        """The clamp must be out-of-place: full backward hooks wrap the projection
        outputs in autograd views, and HF-style clamp_ on them raises. This is the
        property the deleted prepare_model patch used to protect."""
        # fork_rng: deterministic setup without mutating the global RNG stream.
        with torch.random.fork_rng(devices=[]):
            torch.manual_seed(0)
            hf_config = OlmoConfig(
        ...
        # Real backward hooks on the projections — their outputs (the tensors
        # the clamp operates on) become BackwardHookFunction views.
        handles = [
            getattr(attn_bridge, name).register_full_backward_hook(lambda m, gi, go: None)
            for name in ("q", "k", "v")
        ]
        try:
            position_embeddings = identity_rope(seq, cfg.d_head)
