...
class TestBridgeComponentInspection:
    ...
    def test_attention_component_structure(self, bridge):
        ...
        if hasattr(c_attn, 'weight'):
            ...
            assert weight_shape[0] > 0 and weight_shape[1] > 0, f"Weight should have positive dimensions: {weight_shape}"
    ...
    def test_embedding_components(self, bridge):
        ...
        assert wte_shape[0] > 0 and wte_shape[1] > 0, "Token embedding should have positive dimensions"
        ...
        assert wpe_shape[1] == wte_shape[1], "Position and token embeddings should have same hidden dimension"
    ...
    def test_lm_head_structure(self, bridge):
        ...
        assert lm_head_shape[0] == wte_shape[0], "LM head and token embedding should have same vocab size"
    ...
    def test_component_types(self, bridge):
        ...
        assert isinstance(transformer.wpe, torch.nn.Module), "Position embedding should be nn.Module"
    ...
    def test_parameter_devices(self, bridge):
        ...
        assert transformer.wpe.weight.device == expected_device, "Position embedding should be on CPU"

        # Check layer parameters
        layer_0 = transformer.h[0]
        for name, param in layer_0.named_parameters():
            assert param.device == expected_device, f"Parameter {name} should be on CPU, got {param.device}"

        # Check LM head
        assert bridge.original_model.lm_head.weight.device == expected_device, "LM head should be on CPU"
    ...
    def test_parameter_dtypes(self, bridge):
        ...
        assert transformer.wte.weight.dtype in valid_dtypes, f"Token embedding dtype: {transformer.wte.weight.dtype}"
        assert transformer.wpe.weight.dtype in valid_dtypes, f"Position embedding dtype: {transformer.wpe.weight.dtype}"

        # Check layer 0 parameters
        for name, param in transformer.h[0].named_parameters():
            assert param.dtype in valid_dtypes, f"Parameter {name} has unexpected dtype: {param.dtype}"
    ...
    def test_model_configuration_accessible(self, bridge):
        ...
        assert config.n_layer == actual_layers, f"Config layers ({config.n_layer}) should match actual ({actual_layers})"