...
# attention patterns are invariant under its weight processing (fold/center are
# function-preserving), so no_processing keeps the goldens valid — and fold_ln
# currently cannot handle this model's mid-MLP solu_ln layout on the bridge.
model = TransformerBridge.boot_tl_legacy(MODEL, device="cpu")
model.enable_compatibility_mode(no_processing=True, disable_warnings=True)
