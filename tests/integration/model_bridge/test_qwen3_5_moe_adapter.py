...
class TestQwen3_5MoeBridgeCreation:
    def test_routes_to_multimodal_moe_adapter(self, bridge):
        from transformer_lens.model_bridge.supported_architectures.qwen3_5_moe import (
            Qwen3_5MoeMultimodalArchitectureAdapter,
        )

        assert isinstance(bridge.adapter, Qwen3_5MoeMultimodalArchitectureAdapter)
        assert bridge.cfg.is_multimodal is True

    def test_has_vision_and_language_components(self, bridge):
    ...
    def test_hybrid_layer_types_respected(self, bridge):
        """Layer 0 is GatedDeltaNet (linear_attn), layer 1 full attention in the tiny checkpoint."""
        layer_types = bridge.original_model.config.text_config.layer_types
...
class TestQwen3_5MoeHooks:
    def test_moe_mlp_and_hybrid_attention_hooks_fire(self, bridge, sample_tokens):
    ...
    def test_moe_block_is_delegated_hf_module(self, bridge):
