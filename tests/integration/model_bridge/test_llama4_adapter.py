...
class TestLlama4BridgeCreation:
    def test_adapter_selected(self, bridge):
        ...
        assert isinstance(bridge.adapter, Llama4ArchitectureAdapter)
...
class TestLlama4Hooks:
    def test_hooks_fire_in_tl_shapes(self, bridge, sample_tokens):
    ...
    def test_router_scores_hook(self, bridge, sample_tokens):
        captured = {}
    ...
    def test_backward_through_moe(self, bridge, sample_tokens):
        ...
        with bridge.hooks(bwd_hooks=[("blocks.0.mlp.hook_out", grab_grad)]):
            out = bridge(sample_tokens)
            out.sum().backward()
