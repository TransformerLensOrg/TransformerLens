...
opaque, so `ssm_states` is never hooked. The only
...
class TestMamba2BridgeCreation:
    ...
    def test_d_mlp_is_zero(self, mamba2_bridge):
        """in_proj is a 3-way split: projection_size = intermediate + conv_dim + num_heads.
...
class TestMamba2StatefulGeneration:
    """bridge.generate() runs a dedicated stateful loop with Mamba2Cache.
    Tokens match HF's native generate() exactly, and projection hooks fire
    on every step so interventions are possible.
    """
