...
class TestDefaultFoldingBehavior:
    ...
    def test_default_bridge_is_unfolded(self, model_name, device, test_tokens, reference_unfolded, reference_folded):
        ...
        assert not hasattr(bridge_default, 'blocks'), "Default bridge should not have processed components"
        assert not hasattr(bridge_default, '_hf_processing_flags'), "Default bridge should not have processing flags"
        ...
        assert unfolded_diff < 1e-5, f"Default bridge should closely match unfolded reference (diff: {unfolded_diff:.10f})"

    def test_enable_compatibility_mode_applies_folding(self, model_name, device, test_tokens, reference_folded):
        ...
        assert hasattr(bridge_folded, 'blocks'), "Bridge should have processed components after enable_compatibility_mode"
        assert hasattr(bridge_folded, '_hf_processing_flags'), "Bridge should have processing flags after enable_compatibility_mode"
        ...
        assert folded_diff < 1e-8, f"Folded bridge should exactly match folded reference (diff: {folded_diff:.10f})"

    def test_no_processing_flag_prevents_folding(self, model_name, device, test_tokens, reference_unfolded):
        ...
        assert not hasattr(bridge_no_proc, 'blocks'), "Bridge should not have processed components with no_processing=True"
        ...
        assert unfolded_diff < 1e-5, f"no_processing bridge should closely match unfolded reference (diff: {unfolded_diff:.10f})"
    ...
    def test_behavior_consistency(self, model_name, device, test_tokens):
        ...
        assert default_vs_no_proc < 1e-10, f"Default and no_processing should be identical (diff: {default_vs_no_proc:.10f})"

        # Default should be different from folded (folding changes behavior)
        assert default_vs_folded > 1e-6, f"Default and folded should be different (diff: {default_vs_folded:.10f})"

        # Folded should be different from no_processing (flag prevents folding)
        assert folded_vs_no_proc > 1e-6, f"Folded and no_processing should be different (diff: {folded_vs_no_proc:.10f})"

    def test_folding_impact_magnitude(self, model_name, device, test_tokens, reference_unfolded, reference_folded):
        ...
        assert folding_impact > 1e-7, f"Folding should have measurable impact (impact: {folding_impact:.10f})"
