...
class TestSeedOssBridgeCreation:
    def test_adapter_selected(self, bridge):
        ...
        assert isinstance(bridge.adapter, SeedOssArchitectureAdapter)
...
class TestSeedOssBiases:
    def test_config_gated_attention_bias_delegates(self, bridge):
...
class TestSeedOssInferenceMode:
    def test_wrappers_adopt_eval_mode_and_forward_is_deterministic(self, bridge, sample_tokens):
        ...
        assert bridge.blocks[0].attn.training is False
        with torch.no_grad():
            first = bridge(sample_tokens)
            second = bridge(sample_tokens)
