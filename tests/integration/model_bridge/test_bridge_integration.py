...
def test_generate_with_kv_cache():
    ...
    assert len(output_with_cache) > len(prompt), "Generated text with KV cache should be longer than the prompt"
    assert len(output_without_cache) > len(prompt), "Generated text without KV cache should be longer than the prompt"
