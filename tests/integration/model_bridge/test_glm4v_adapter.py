...
class TestGlm4vGeneration:
    def test_generate_after_multimodal_forward(self, glm4v_bridge, snapshot_path):
        ...
        bridge_inputs = {k: v for k, v in inputs.items() if k != "input_ids"}
        with torch.no_grad():
            glm4v_bridge(inputs["input_ids"], **bridge_inputs)

        text_out = glm4v_bridge.generate("Hello", max_new_tokens=5, do_sample=False, verbose=False)
