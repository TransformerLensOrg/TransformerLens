...
def test_activation_cache_hook_aliases():
    """Test that ActivationCache handles hook aliases for backward compatibility.

    This ensures that old hook names like 'hook_pattern' can still be accessed
    even when the actual hook is stored under the modern name 'hook_attention_weights'.
    """
    ...
    # Test 1: Direct access with modern hook name works
    modern_result = cache["blocks.0.attn.hook_attention_weights"]
    assert isinstance(modern_result, torch.Tensor), "Modern hook name should return tensor"

    # Test 2: Access with old hook name should find the alias
    legacy_result = cache["blocks.0.attn.hook_pattern"]
    assert isinstance(
        legacy_result, torch.Tensor
    ), "Legacy hook name should return tensor via alias"

    # Test 3: Both should return the same data (same tensor object)
    assert torch.equal(
        modern_result, legacy_result
    ), "Modern and legacy access should return identical data"

    # Test 4: Shorthand access should work with old pattern name
    shorthand_result = cache[("pattern", 0)]
    assert torch.equal(modern_result, shorthand_result), "Shorthand pattern access should work"

    # Test 5: Multiple layers should work
    layer1_modern = cache["blocks.1.attn.hook_attention_weights"]
    layer1_legacy = cache["blocks.1.attn.hook_pattern"]
    assert torch.equal(layer1_modern, layer1_legacy), "Hook aliases should work for all layers"

    # Test 6: Non-aliased hooks should still work normally
    k_result = cache["blocks.0.attn.hook_k"]
    assert isinstance(k_result, torch.Tensor), "Non-aliased hooks should work normally"

    # Test 7: KeyError should still be raised for truly missing hooks
    with pytest.raises(KeyError):
        _ = cache["blocks.0.attn.hook_nonexistent"]
