...
def test_get_device_for_block_index(gpt2_medium_on_4_devices):
    ...
    for i in range(n_layers):
        expected_device = torch.device(config_device.type,
                                       i // layers_per_device)
    ...
    for i in range(n_layers):
        ...
        assert get_device_for_block_index(i, config,
                                          device_override) == expected_device

    # Test with explicit torch.device object
    device_override_obj = torch.device("cuda")
    for i in range(n_layers):
        expected_device = torch.device(device_override_obj.type,
                                       i // layers_per_device)
        assert get_device_for_block_index(
            i, config, device_override_obj) == expected_device
...
def test_device_separation_and_cache(gpt2_medium_on_1_device, n_devices):
    ...
    model_n_devices = HookedTransformer.from_pretrained("gpt2-medium",
                                                        fold_ln=False,
                                                        n_devices=n_devices)
    ...
    loss_n_devices = model_n_devices(model_description_text,
                                     return_type="loss")
    ...
    for device in cuda_devices:
        prop_device = n_params_on_device[device] / len(
            model_n_devices.state_dict())
