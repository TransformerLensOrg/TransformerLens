...
- Requires a CUDA GPU and a working `vllm` install. There is no `[vllm]` packaging extra (vLLM's dependency pins conflict with other extras); the internals are validated against `vllm==0.20.2` — see [`sources/vllm/internals.py`](https://github.com/TransformerLensOrg/TransformerLens/blob/dev-4.x/transformer_lens/model_bridge/sources/vllm/internals.py) before bumping.
