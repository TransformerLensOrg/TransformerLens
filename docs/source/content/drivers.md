...
- `pipeline_parallel_size=2` enables single-node pipeline parallelism (**experimental — not yet GPU-validated**): each stage owns a disjoint layer slice, so the driver merges per-rank capture reads (fail-loud if a requested hook is served by no rank) and each rank applies only the intervention specs whose hooks it owns; the norm weight and unembedding are gathered from the last stage for logit reconstruction. The first-forward layout check also verifies fire counters stay proportional under vLLM's PP microbatching. Composes with `tensor_parallel_size`; same `enable_batching` restriction.
