...
def get_bridge_params(bridge) -> Dict[str, torch.Tensor]:
    ...
    try:
        ...
        params_dict['embed.W_E'] = torch.zeros(bridge.cfg.d_vocab, bridge.cfg.d_model, device=device, dtype=dtype)
    try:
        params_dict['pos_embed.W_pos'] = bridge.pos_embed.weight
    except AttributeError:
        device, dtype = _get_device_dtype()
        params_dict['pos_embed.W_pos'] = torch.zeros(bridge.cfg.n_ctx, bridge.cfg.d_model, device=device, dtype=dtype)
    for layer_idx in range(bridge.cfg.n_layers):
        if layer_idx >= len(bridge.blocks):
            raise ValueError(f'Configuration mismatch: cfg.n_layers={bridge.cfg.n_layers} but only {len(bridge.blocks)} blocks found. Layer {layer_idx} does not exist.')
        ...
        try:
            ...
            if w_q.shape == (bridge.cfg.d_model, bridge.cfg.d_model):
                ...
                if w_k.shape == (bridge.cfg.d_model, bridge.cfg.d_model):
                ...
                elif w_k.shape == (bridge.cfg.d_head, bridge.cfg.d_model) or w_k.shape == (bridge.cfg.d_model // bridge.cfg.n_heads, bridge.cfg.d_model):
                    ...
                    device, dtype = _get_device_dtype()
                    w_k = torch.zeros(bridge.cfg.n_heads, bridge.cfg.d_model, bridge.cfg.d_head, device=device, dtype=dtype)
                if w_v.shape == (bridge.cfg.d_model, bridge.cfg.d_model):
                    w_v = w_v.reshape(bridge.cfg.n_heads, bridge.cfg.d_model, d_head)
                elif w_v.shape == (bridge.cfg.d_head, bridge.cfg.d_model) or w_v.shape == (bridge.cfg.d_model // bridge.cfg.n_heads, bridge.cfg.d_model):
                    ...
                    device, dtype = _get_device_dtype()
                    w_v = torch.zeros(bridge.cfg.n_heads, bridge.cfg.d_model, bridge.cfg.d_head, device=device, dtype=dtype)
            ...
            if block.attn.q.bias is not None:
                params_dict[f'blocks.{layer_idx}.attn.b_Q'] = block.attn.q.bias.reshape(bridge.cfg.n_heads, -1)
            else:
                device, dtype = _get_device_dtype()
                params_dict[f'blocks.{layer_idx}.attn.b_Q'] = torch.zeros(bridge.cfg.n_heads, bridge.cfg.d_head, device=device, dtype=dtype)
            if block.attn.k.bias is not None:
                n_kv_heads = bridge.cfg.n_heads
                if hasattr(bridge.cfg, 'n_key_value_heads') and isinstance(bridge.cfg.n_key_value_heads, int):
                    n_kv_heads = bridge.cfg.n_key_value_heads
                params_dict[f'blocks.{layer_idx}.attn.b_K'] = block.attn.k.bias.reshape(n_kv_heads, -1)
            else:
                device, dtype = _get_device_dtype()
                n_kv_heads = bridge.cfg.n_heads
                if hasattr(bridge.cfg, 'n_key_value_heads') and isinstance(bridge.cfg.n_key_value_heads, int):
                    n_kv_heads = bridge.cfg.n_key_value_heads
                params_dict[f'blocks.{layer_idx}.attn.b_K'] = torch.zeros(n_kv_heads, bridge.cfg.d_head, device=device, dtype=dtype)
            if block.attn.v.bias is not None:
                n_kv_heads = bridge.cfg.n_heads
                if hasattr(bridge.cfg, 'n_key_value_heads') and isinstance(bridge.cfg.n_key_value_heads, int):
                    n_kv_heads = bridge.cfg.n_key_value_heads
                params_dict[f'blocks.{layer_idx}.attn.b_V'] = block.attn.v.bias.reshape(n_kv_heads, -1)
            else:
                device, dtype = _get_device_dtype()
                n_kv_heads = bridge.cfg.n_heads
                if hasattr(bridge.cfg, 'n_key_value_heads') and isinstance(bridge.cfg.n_key_value_heads, int):
                    n_kv_heads = bridge.cfg.n_key_value_heads
                params_dict[f'blocks.{layer_idx}.attn.b_V'] = torch.zeros(n_kv_heads, bridge.cfg.d_head, device=device, dtype=dtype)
            if block.attn.o.bias is not None:
                params_dict[f'blocks.{layer_idx}.attn.b_O'] = block.attn.o.bias
            else:
                device, dtype = _get_device_dtype()
                params_dict[f'blocks.{layer_idx}.attn.b_O'] = torch.zeros(bridge.cfg.d_model, device=device, dtype=dtype)
        except AttributeError:
            device, dtype = _get_device_dtype()
            ...
            params_dict[f'blocks.{layer_idx}.attn.W_Q'] = torch.zeros(*expected_qkv_shape, device=device, dtype=dtype)
        ...
        try:
            ...
            device, dtype = _get_device_dtype()
            d_mlp = bridge.cfg.d_mlp if bridge.cfg.d_mlp is not None else 4 * bridge.cfg.d_model
            params_dict[f'blocks.{layer_idx}.mlp.W_in'] = torch.zeros(bridge.cfg.d_model, d_mlp, device=device, dtype=dtype)
    ...
    try:
        ...
        device, dtype = _get_device_dtype()
        params_dict['unembed.W_U'] = torch.zeros(bridge.cfg.d_model, bridge.cfg.d_vocab, device=device, dtype=dtype)
    try:
