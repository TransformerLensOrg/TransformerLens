...
class T5BlockBridge(GeneralizedComponent):
    ...
    def __init__(self, name: str, config: Optional[Any]=None, submodules: Optional[Dict[str, GeneralizedComponent]]=None, is_decoder: bool=False):
    ...
    def _patch_t5_block_forward(self):
        ...
        def patched_forward(block_self, hidden_states, attention_mask=None, position_bias=None, encoder_hidden_states=None, encoder_attention_mask=None, encoder_decoder_position_bias=None, layer_head_mask=None, cross_attn_layer_head_mask=None, past_key_value=None, use_cache=False, output_attentions=False, return_dict=True, **kwargs):
            ...
            if past_key_value is not None:
                ...
                if len(past_key_value) != expected_num_past_key_values:
                    raise ValueError(f'There should be {expected_num_past_key_values} past states. Got {len(past_key_value)}.')
            ...
            self_attention_outputs = layers[0](hidden_states, attention_mask=attention_mask, position_bias=position_bias, layer_head_mask=layer_head_mask, past_key_value=self_attn_past_key_value, use_cache=use_cache, output_attentions=output_attentions)
            ...
            if is_decoder_block and encoder_hidden_states is not None:
                cross_attention_outputs = layers[1](hidden_states, key_value_states=encoder_hidden_states, attention_mask=encoder_attention_mask, position_bias=encoder_decoder_position_bias, layer_head_mask=cross_attn_layer_head_mask, past_key_value=cross_attn_past_key_value, use_cache=use_cache, output_attentions=output_attentions)
