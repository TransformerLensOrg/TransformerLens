...
class LinearBridge(GeneralizedComponent):
    ...
    def __init__(self, name: str, config: Optional[Any]=None, submodules: Optional[Dict[str, GeneralizedComponent]]=None, conversion_rule: Optional[BaseHookConversion]=None) -> None:
    ...
    def set_processed_weights(self, weights: Mapping[str, torch.Tensor | None]) -> None:
        ...
        if weight.ndim == 3:
            ...
            if dim1 > dim2:
                ...
                weight = einops.rearrange(weight, 'n_heads d_head d_model -> (n_heads d_head) d_model')
