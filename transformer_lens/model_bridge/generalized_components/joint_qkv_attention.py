...
class JointQKVAttentionBridge(AttentionBridge):
    ...
    def __init__(self, name: str, config: Any, split_qkv_matrix: Callable, submodules: Optional[Dict[str, GeneralizedComponent]]=None, qkv_conversion_rule: Optional[BaseHookConversion]=None, attn_conversion_rule: Optional[BaseHookConversion]=None, pattern_conversion_rule: Optional[BaseHookConversion]=None, requires_position_embeddings: bool=False, requires_attention_mask: bool=False):
        ...
        """
        super().__init__(name, config, submodules=submodules, conversion_rule=attn_conversion_rule, pattern_conversion_rule=pattern_conversion_rule, requires_position_embeddings=requires_position_embeddings, requires_attention_mask=requires_attention_mask)
        ...
        class ConditionalRearrangeConversion(BaseHookConversion):
            ...
            def handle_conversion(self, input_value: torch.Tensor, *full_context) -> torch.Tensor:
                if input_value.ndim == 4:
                    ...
                    return einops.rearrange(input_value, self.pattern, num_attention_heads=self.n_heads)
                else:
                    raise ValueError(f'Expected 3D or 4D tensor, got {input_value.ndim}D with shape {input_value.shape}')

            def revert(self, input_value: torch.Tensor, *full_context) -> torch.Tensor:
                if input_value.ndim == 3:
                    return input_value
                elif input_value.ndim == 4:
                    return einops.rearrange(input_value, 'batch seq num_attention_heads d_head -> batch seq (num_attention_heads d_head)', num_attention_heads=self.n_heads)
                else:
                    raise ValueError(f'Expected 3D or 4D tensor, got {input_value.ndim}D with shape {input_value.shape}')
    ...
    def set_original_component(self, original_component: torch.nn.Module) -> None:
        ...
        q_transformation, k_transformation, v_transformation = self.split_qkv_matrix(original_component)
    ...
    def _forward_folded(self, *args: Any, **kwargs: Any) -> Any:
        ...
        if hooked_weights_available:
            if hasattr(self, '_W_Q') and hasattr(self, '_W_K') and hasattr(self, '_W_V'):
                ...
                W_Q_flat = W_Q.transpose(0, 1).contiguous().view(cfg.d_model, -1)
                W_K_flat = W_K.transpose(0, 1).contiguous().view(cfg.d_model, -1)
                W_V_flat = W_V.transpose(0, 1).contiguous().view(cfg.d_model, -1)
                ...
                qkv = original_attn.c_attn(hidden_states)
    ...
    def _compatibility_mode_forward_with_hooks(self, *args: Any, **kwargs: Any) -> Any:
        ...
        if not self._hooked_weights_extracted or not hasattr(self, '_W_Q') or (not hasattr(self, '_W_K')) or (not hasattr(self, '_W_V')) or (self._W_Q is None) or (self._W_K is None) or (self._W_V is None):
            return super().forward(*args, **kwargs)
        if self._b_Q is None:
            self._b_Q = torch.zeros(self._W_Q.shape[0], self._W_Q.shape[2], dtype=self._W_Q.dtype, device=self._W_Q.device)
        if self._b_K is None:
            self._b_K = torch.zeros(self._W_K.shape[0], self._W_K.shape[2], dtype=self._W_K.dtype, device=self._W_K.device)
        if self._b_V is None:
            self._b_V = torch.zeros(self._W_V.shape[0], self._W_V.shape[2], dtype=self._W_V.dtype, device=self._W_V.device)
        if hasattr(self, '_ln1') and self._ln1 is not None:
        ...
        if hasattr(self, '_W_O') and self._W_O is not None:
            w = einops.rearrange(self._W_O, 'head_index d_head d_model -> d_model (head_index d_head)')
    ...
    def _reconstruct_attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, **kwargs) -> tuple:
    ...
    def _apply_qkv_projection_with_functional_linear(self, hidden_states: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        """
        print(f'CALLED: {__file__}::JointQKVAttentionBridge._apply_qkv_projection_with_functional_linear')
        original_component = self.original_component
        assert original_component is not None
        assert self.config is not None
    ...
    def _apply_output_projection_with_functional_linear(self, attn_output: torch.Tensor) -> torch.Tensor:
        ...
        """
        print(f'CALLED: {__file__}::JointQKVAttentionBridge._apply_output_projection_with_functional_linear')
        original_component = self.original_component
        assert original_component is not None
    ...
    def set_processed_weights(self, weights: Mapping[str, torch.Tensor | None]) -> None:
        ...
        if hasattr(self, 'o') and self.o is not None and hasattr(self.o, 'set_processed_weights') and (W_O is not None):
    ...
    def process_weights(self, fold_ln: bool=False, center_writing_weights: bool=False, center_unembed: bool=False, fold_value_biases: bool=False, refactor_factored_attn_matrices: bool=False) -> None:
        ...
        qkv_bias = einops.rearrange(qkv_bias_tensor, '(qkv index head)->qkv index head', qkv=3, index=self.config.n_heads, head=self.config.d_head)
            ...
            for name, module in self.submodules.items():
                if hasattr(module, 'name') and module.name == 'c_proj':
                    ...
                    if proj_submodule is not None:
                        ...
                        W_O = einops.rearrange(W_O, '(i h) m->i h m', i=self.config.n_heads)
                    break
        self._processed_weights = {'W_Q': W_Q, 'W_K': W_K, 'W_V': W_V, 'b_Q': b_Q, 'b_K': b_K, 'b_V': b_V}
        if W_O is not None:
    ...
    def get_processed_state_dict(self) -> Dict[str, torch.Tensor]:
        ...
        """
        print(f'CALLED: {__file__}::JointQKVAttentionBridge.get_processed_state_dict')
    ...
    def custom_weight_processing(self, hf_state_dict: Dict[str, torch.Tensor], component_prefix: str, **processing_kwargs) -> Dict[str, torch.Tensor]:
        ...
        if qkv_weight_key in hf_state_dict:
            ...
            processed_weights['W_Q'] = einops.rearrange(q_weight, 'd_model (n_heads d_head) -> n_heads d_model d_head', n_heads=n_heads, d_head=d_head)
            processed_weights['W_K'] = einops.rearrange(k_weight, 'd_model (n_heads d_head) -> n_heads d_model d_head', n_heads=n_heads, d_head=d_head)
            processed_weights['W_V'] = einops.rearrange(v_weight, 'd_model (n_heads d_head) -> n_heads d_model d_head', n_heads=n_heads, d_head=d_head)
        if qkv_bias_key in hf_state_dict:
            ...
            n_heads = self.config.n_heads
            d_head = self.config.d_head
            processed_weights['b_Q'] = einops.rearrange(q_bias, '(n_heads d_head) -> n_heads d_head', n_heads=n_heads, d_head=d_head)
            processed_weights['b_K'] = einops.rearrange(k_bias, '(n_heads d_head) -> n_heads d_head', n_heads=n_heads, d_head=d_head)
            processed_weights['b_V'] = einops.rearrange(v_bias, '(n_heads d_head) -> n_heads d_head', n_heads=n_heads, d_head=d_head)
        ...
        if out_weight_key in hf_state_dict:
            ...
            processed_weights['W_O'] = einops.rearrange(out_weight, '(n_heads d_head) d_model -> n_heads d_head d_model', n_heads=n_heads, d_head=d_head)
    ...
    def _extract_hooked_transformer_weights(self) -> None:
        ...
        if hasattr(self, '_processed_W_Q') and hasattr(self, '_processed_W_K') and hasattr(self, '_processed_W_V'):
        ...
        try:
            ...
            reference_model = HookedTransformer.from_pretrained(model_name, device=device, fold_ln=True, center_writing_weights=True, center_unembed=True, fold_value_biases=True, refactor_factored_attn_matrices=False)
            layer_num = 0
            current = self
        ...
        if self._processed_weights is None:
            try:
                self.process_weights(fold_ln=True, center_writing_weights=True, center_unembed=True, fold_value_biases=True, refactor_factored_attn_matrices=False)
