...
class MLPBridge(GeneralizedComponent):
    ...
    property_aliases = {'W_gate': 'gate.weight', 'b_gate': 'gate.bias', 'W_in': 'in.weight', 'b_in': 'in.bias', 'W_out': 'out.weight', 'b_out': 'out.bias'}

    def __init__(self, name: Optional[str], config: Optional[Any]=None, submodules: Optional[Dict[str, GeneralizedComponent]]={}):
    ...
    def forward(self, *args, **kwargs) -> torch.Tensor:
        ...
        if hasattr(self, '_use_processed_weights') and self._use_processed_weights:
            ...
            if hasattr(self, '_processed_W_in') and hasattr(self, '_processed_W_out'):
                b_in = self._processed_b_in if self._processed_b_in is not None else torch.zeros(self._processed_W_in.shape[-1], device=hidden_states.device, dtype=hidden_states.dtype)
                ...
                b_out = self._processed_b_out if self._processed_b_out is not None else torch.zeros(self._processed_W_out.shape[-1], device=hidden.device, dtype=hidden.dtype)
