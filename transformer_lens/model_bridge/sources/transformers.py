...
import os
import torch
from transformers import AutoConfig, AutoModel, AutoModelForCausalLM, AutoModelForSeq2SeqLM, AutoTokenizer, PreTrainedTokenizerBase
...
def determine_architecture_from_hf_config(hf_config):
    ...
    if hasattr(hf_config, 'model_type'):
        ...
        model_type_mappings = {'gpt2': 'GPT2LMHeadModel', 'llama': 'LlamaForCausalLM', 'mistral': 'MistralForCausalLM', 'mixtral': 'MixtralForCausalLM', 'gemma': 'GemmaForCausalLM', 'gemma2': 'Gemma2ForCausalLM', 'gemma3': 'Gemma3ForCausalLM', 'bert': 'BertForMaskedLM', 'bloom': 'BloomForCausalLM', 'gptj': 'GPTJForCausalLM', 'gpt_neo': 'GPTNeoForCausalLM', 'gpt_neox': 'GPTNeoXForCausalLM', 'opt': 'OPTForCausalLM', 'phi': 'PhiForCausalLM', 'phi3': 'Phi3ForCausalLM', 'qwen': 'QwenForCausalLM', 'qwen2': 'Qwen2ForCausalLM', 't5': 'T5ForConditionalGeneration'}
    ...
    for arch in architectures:
        if arch in SUPPORTED_ARCHITECTURES:
            return arch
    raise ValueError(f"Could not determine supported architecture from config. Available architectures: {list(SUPPORTED_ARCHITECTURES.keys())}, Config architectures: {architectures}, Model type: {getattr(hf_config, 'model_type', None)}")
...
def get_hf_model_class_for_architecture(architecture: str):
    ...
    """
    seq2seq_architectures = {'T5ForConditionalGeneration', 'BartForConditionalGeneration', 'MBartForConditionalGeneration', 'MarianMTModel', 'PegasusForConditionalGeneration', 'BlenderbotForConditionalGeneration', 'BlenderbotSmallForConditionalGeneration'}
    masked_lm_architectures = {'BertForMaskedLM', 'RobertaForMaskedLM', 'DistilBertForMaskedLM', 'AlbertForMaskedLM', 'ElectraForMaskedLM'}
...
def boot(model_name: str, hf_config_overrides: dict | None=None, device: str | torch.device | None=None, dtype: torch.dtype=torch.float32, tokenizer: PreTrainedTokenizerBase | None=None, load_weights: bool=True) -> TransformerBridge:
    ...
    if tokenizer is not None:
        ...
        tokenizer = setup_tokenizer(AutoTokenizer.from_pretrained(model_name, add_bos_token=True, use_fast=use_fast, token=huggingface_token if len(huggingface_token) > 0 else None), default_padding_side=default_padding_side)
    if tokenizer is not None:
...
def setup_tokenizer(tokenizer, default_padding_side=None):
    ...
    """
    assert isinstance(tokenizer, PreTrainedTokenizerBase), f'{type(tokenizer)} is not a supported tokenizer, please use PreTrainedTokenizer or PreTrainedTokenizerFast'
    assert default_padding_side in ['right', 'left', None], f"padding_side must be 'right', 'left' or 'None', got {default_padding_side}"
