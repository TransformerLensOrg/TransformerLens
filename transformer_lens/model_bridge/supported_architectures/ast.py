...
class ASTEmbed(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.patch_embeddings = nn.Conv2d(
            in_channels=1, out_channels=cfg.d_model, kernel_size=patch_size, stride=stride
        )
...
class ASTAdapter(ArchitectureAdapter):
    ...
    @classmethod
    def convert_weights(
        cls, hf_state_dict: Dict[str, torch.Tensor], tl_config: HookedTransformerConfig
        ...
        if (
            ...
            state_dict["embed.patch_embeddings.weight"] = hf_state_dict[
                "audio_spectrogram_transformer.embeddings.patch_embeddings.projection.weight"
            ]
            state_dict["embed.patch_embeddings.bias"] = hf_state_dict[
        ...
        if "audio_spectrogram_transformer.embeddings.position_embeddings" in hf_state_dict:
            state_dict["pos_embed.W_pos"] = hf_state_dict[
        ...
        if "audio_spectrogram_transformer.embeddings.cls_token" in hf_state_dict:
            state_dict["embed.cls_token"] = hf_state_dict[
        ...
        if "audio_spectrogram_transformer.embeddings.distillation_token" in hf_state_dict:
            state_dict["embed.distillation_token"] = hf_state_dict[
        ...
        if "audio_spectrogram_transformer.layernorm.weight" in hf_state_dict:
            state_dict["ln_final.w"] = hf_state_dict[
    ...
    @staticmethod
    def _reshape_weight(
        weight: torch.Tensor, cfg: HookedTransformerConfig, is_output: bool = False
    ) -> torch.Tensor:
