...
class Attention(nn.Module):
    def __init__(
        self,
        cfg: Union[Dict, TransformerLensConfig],
        ...
        Args:
            cfg (Union[Dict, TransformerLensConfig]): Config
        ...
        # See TransformerLensConfig for more details.
        if self.cfg.positional_embedding_type == "shortformer":
            ...
            # Applies a rotation to each two-element chunk of keys and queries pre dot producting to bake in relative position. See TransformerLensConfig for details
    ...
    def forward(
        ...
        """
        shortformer_pos_embed is only used if self.cfg.positional_embedding_type == "shortformer", else defaults to None and is irrelevant. See TransformerLensConfig for more details
        ...
        if self.cfg.positional_embedding_type in ["standard", "rotary"]:
            ...
            # Weird shortformer attention see TransformerLensConfig for details
...
class TransformerBlock(nn.Module):
    def __init__(self, cfg: Union[Dict, TransformerLensConfig], block_index):
        super().__init__()
        if isinstance(cfg, Dict):
            cfg = TransformerLensConfig.from_dict(cfg)
        self.cfg = cfg
    ...
    def forward(
        ...
        Args:
            ...
            shortformer_pos_embed (torch.Tensor, optional): Only used for positional_embeddings_type == "shortformer". The positional embeddings. See TransformerLensConfig for details. Defaults to None.
