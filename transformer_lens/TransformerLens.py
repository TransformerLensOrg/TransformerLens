...
class TransformerLens(HookedRootModule):
    """
    ...
    It can have a pretrained Transformer's weights automatically loaded in via the TransformerLens.from_pretrained class method. It can also be instantiated with randomly initialized weights via __init__ and being passed a dict or TransformerLensConfig object.
    """
    ...
    def __init__(
        ...
        """
        Model initialization. Note that if you want to load the model from pretrained weights, you should use the TransformerLens.from_pretrained() class method instead of this one.

        cfg Union[TransformerLensConfig, Dict]: The config to use for the
            model.
        ...
        if isinstance(cfg, Dict):
            ...
            raise ValueError(
                "Please pass in a config dictionary or TransformerLensConfig object. If you want to load a pretrained model, use TransformerLens.from_pretrained() instead."
            )
        self.cfg = cfg
    ...
    def forward(
        ...
        if self.cfg.positional_embedding_type == "standard":
            ...
            # If we're using shortformer style attention, we don't add the positional embedding to the residual stream. See TransformerLensConfig for details
            pos_embed = self.hook_pos_embed(
                self.pos_embed(tokens, pos_offset)
            )  # [batch, pos, d_model]
            residual = embed
            ...
            # Rotary doesn't use positional embeddings, instead they're applied when dot producting keys and queries. See TransformerLensConfig for details
            residual = embed
            shortformer_pos_embed = None
        else:
            raise ValueError(
        ...
        for i, block in enumerate(self.blocks):
            ...
            residual = block(
                ...
                else None,  # Cache is contains a list of TransformerLensKeyValueCache objects, one for each block
    ...
    def run_with_cache(
        ...
        """
        Wrapper around run_with_cache in HookedRootModule. If return_cache_object is True, this will return an ActivationCache object, with a bunch of useful TransformerLens specific methods, otherwise it will return a dictionary of activations as in HookedRootModule.
        """
    ...
    def from_pretrained(
        ...
        **model_kwargs,
        ...
        """Class method to load in a pretrained model weights to the TransformerLens format and optionally to do some processing to make the model easier to interpret. Currently supports loading from most autoregressive HuggingFace models (GPT2, GPTNeo, GPTJ, OPT) and from a range of toy models and SoLU models trained by me (Neel Nanda).
        ...
        Args:
            ...
            model_kwargs (dict, optional): Any additional kwargs to pass to the
                TransformerLens initialization.
        """
        ...
        # Load the config into an TransformerLensConfig object If loading from a
        ...
        # Get the state dict of the model (ie a mapping of parameter names to tensors), processed to match the TransformerLens parameter names.
        state_dict = loading.get_pretrained_state_dict(
            official_model_name, cfg, hf_model
        )

        # Create the TransformerLens object
        ...
        print(f"Finished loading pretrained model {model_name} into TransformerLens!")
    ...
    def load_and_process_state_dict(
        ...
        """Method to load a state dict into the model, and to apply processing to simplify it. The state dict is assumed to be in the TransformerLens format.

        See the relevant method (same name as the flag) for more details on the folding, centering and processing flags.

        Args:
            state_dict (dict): The state dict of the model, in TransformerLens format
            fold_ln (bool, optional): Whether to fold in the LayerNorm weights to the
    ...
    def fold_layer_norm(self, state_dict: Dict[str, torch.Tensor]):
        """Takes in a state dict from a pretrained model, formatted to be consistent with TransformerLens but with LayerNorm weights and biases. Folds these into the neighbouring weights. See TransformerLensConfig for more details
    ...
    def process_weights_(
        ...
        """
        Wrapper around load_and_process_state_dict to allow for in-place processing of the weights. This is useful if using TransformerLens for training, if we then want to analyse a cleaner version of the same model.
        """
    ...
    def generate(
        ...
        # Currently nothing in TransformerLens changes with eval, but this is here in case that changes in the future
        self.eval()
