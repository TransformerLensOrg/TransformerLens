...
def convert_mllama_weights(mllama, cfg: HookedTransformerConfig):
    ...
    for l in range(cfg.n_layers):
        state_dict[f"blocks.{l}.ln1.w"] = mllama.model.layers[l].input_layernorm.weight
        ...
        state_dict[f"blocks.{l}.ln2.w"] = mllama.model.layers[l].post_attention_layernorm.weight

        # in case of quantization,
        # parameters should stay as bitsandbytes.nn.modules.Params4bit
        if not cfg.load_in_4bit:
            state_dict[f"blocks.{l}.mlp.W_in"] = mllama.model.layers[l].mlp.up_proj.weight.T
            state_dict[f"blocks.{l}.mlp.W_gate"] = mllama.model.layers[l].mlp.gate_proj.weight.T
            state_dict[f"blocks.{l}.mlp.W_out"] = mllama.model.layers[l].mlp.down_proj.weight.T
        else:
            state_dict[f"blocks.{l}.mlp.W_in"] = mllama.model.layers[l].mlp.up_proj.weight
            state_dict[f"blocks.{l}.mlp.W_gate"] = mllama.model.layers[l].mlp.gate_proj.weight
            state_dict[f"blocks.{l}.mlp.W_out"] = mllama.model.layers[l].mlp.down_proj.weight
    ...
    state_dict["ln_final.w"] = mllama.model.norm.weight

    state_dict["unembed.W_U"] = mllama.lm_head.weight.T
