...
from transformers.models.mixtral import MixtralModel

from transformer_lens.HookedTransformerConfig import HookedTransformerConfig, MixtralDecoderLayer


def _convert_layer_weights(layer: MixtralDecoderLayer, state_dict: dict) -> dict:
    return state_dict
...
def convert_mixtral_weights(hf_model: any, cfg: HookedTransformerConfig):
    ...
    for l in range(cfg.n_layers):
        state_dict[f"blocks.{l}.ln1.w"] = model.layers[l].input_layernorm.weight

        W_Q = model.layers[l].self_attn.q_proj.weight
        W_K = model.layers[l].self_attn.k_proj.weight
        W_V = model.layers[l].self_attn.v_proj.weight
        ...
        W_O = model.layers[l].self_attn.o_proj.weight
        W_O = einops.rearrange(W_O, "m (n h)->n h m", n=cfg.n_heads)
        state_dict[f"blocks.{l}.attn.W_O"] = W_O

        state_dict[f"blocks.{l}.attn.b_O"] = torch.zeros(cfg.d_model, dtype=cfg.dtype)

        state_dict[f"blocks.{l}.ln2.w"] = model.layers[l].post_attention_layernorm.weight

        state_dict[f"blocks.{l}.mlp.W_gate"] = model.layers[
            l
