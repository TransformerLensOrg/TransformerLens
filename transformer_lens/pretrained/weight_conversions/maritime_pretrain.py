...
def convert_maritime_pretrain_weights(
    ...
    * SwiGLU maps onto gated-MLP naming as gate -> W_gate, up -> W_in,
    ...
    # These are Optional on HookedTransformerConfig but are always set on a
    # config built for this architecture; assert once so the shape arithmetic
    # below is over concrete ints (and a misbuilt config fails loudly here).
    assert cfg.d_mlp is not None, "d_mlp must be set"
    d_mlp: int = cfg.d_mlp
    ...
    for layer in range(cfg.n_layers):
        ...
        if is_moe:
            assert cfg.num_experts is not None  # narrowed by is_moe
            n_experts = cfg.num_experts
            router = _check(
                src(layer, "mlp.router.weight"), (n_experts, d_model), f"{p}.mlp.router"
            )
