...
NEED_REMOTE_CODE_MODELS = ("bigcode/santacoder", "Qwen/Qwen-", "microsoft/phi-1", "microsoft/phi-1_5", "microsoft/phi-2")
...
def convert_phi_weights(phi, cfg: HookedTransformerConfig):
    ...
    for l in range(cfg.n_layers):
        ...
        W_Q = einops.rearrange(W_Q, "(n_head d_head) d_model -> n_head d_model d_head", n_head = cfg.n_heads)
        W_K = einops.rearrange(W_K, "(n_head d_head) d_model  -> n_head d_model d_head", n_head = cfg.n_heads)
        W_V = einops.rearrange(W_V, "(n_head d_head) d_model  -> n_head d_model d_head", n_head = cfg.n_heads)
        state_dict[f"blocks.{l}.attn.W_Q"] = W_Q
        state_dict[f"blocks.{l}.attn.W_K"] = W_K
        state_dict[f"blocks.{l}.attn.W_V"] = W_V
        ...
        W_O = einops.rearrange(W_O, "d_model (n_head d_head) -> n_head d_head d_model", n_head = cfg.n_heads)
