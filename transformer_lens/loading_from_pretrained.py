...
def convert_hf_model_config(official_model_name: str):
    ...
    in the TransformerLensConfig format.
...
def convert_neel_model_config(official_model_name: str):
    ...
    AutoConfig is not supported, because these models are in the TransformerLens format, so we directly download and load the json.
    """
    official_model_name = get_official_model_name(official_model_name)
...
def get_pretrained_model_config(
    ...
    """Returns the pretrained model config as an TransformerLensConfig object.
    ...
    Args:
        ...
        fold_ln (bool, optional): Whether to fold the layer norm into the
            subsequent linear layers (see TransformerLens.fold_layer_norm for
    ...
    cfg = TransformerLensConfig.from_dict(cfg_dict)
    return cfg
...
def get_pretrained_state_dict(
    ...
    have the TransformerLens parameter names and shapes. Supports checkpointed
...
def convert_state_dict(
    ...
    compatible with TransformerLens."""
    official_model_name = get_official_model_name(official_model_name)
...
# Convert state dicts
def convert_gpt2_weights(gpt2, cfg: TransformerLensConfig):
    state_dict = {}
...
def convert_gptj_weights(gptj, cfg: TransformerLensConfig):
    state_dict = {}
...
def convert_opt_weights(opt, cfg: TransformerLensConfig):
    state_dict = {}
...
def convert_neel_solu_old_weights(state_dict: dict, cfg: TransformerLensConfig):
    """
    Converts the weights of my old SoLU models to the TransformerLens format.
    ...
    [dim_out, dim_in]) while TransformerLens does right facing (ie [dim_in,
    dim_out]).
