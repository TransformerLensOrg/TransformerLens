...
def merge_quantiziation_fields(
    conversion_set: WeightConversionSet, quantiziation_fields: FIELD_SET
) -> WeightConversionSet:
    for transformer_lens_field in quantiziation_fields:
        ...
        if isinstance(new_field, tuple) and isinstance(new_field[1], WeightConversionSet):
            if not isinstance(existing_field, tuple) or not isinstance(
                existing_field[1], WeightConversionSet
            ):
                raise RuntimeError(
            ...
            conversion_set.fields[transformer_lens_field] = (
                ...
                merge_quantiziation_fields(
                    existing_conversion_set, replacement_conversion_set.fields
                ),
            )
