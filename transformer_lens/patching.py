...
def generic_activation_patch(
    ...
    patching_metric: Callable[[TT[T.batch, T.pos, T.d_vocab]], float],
    ...
    index_df: The dataframe of indices, columns are axis names and each row is a tuple of indices. Will be inferred from index_axis_names if not given.
    return_index_df: A Boolean flag for whether to return the dataframe of indices too

    Returns
    patched_output: The tensor of the patching metric for each patch, has one dimension for each index dimension.
    ...
    for index_row in index_df.iterrows():
        ...
        patched_metric_output[tuple(index)] = patching_metric(patched_logits)
...
# %%
attn_head_out_act_patch = partial(
    generic_activation_patch,
    patch_setter = layer_pos_head_vector_patch_setter,
    activation_name = "z",
    index_axis_names = ("layer", "pos", "head")
)
attn_head_q_act_patch = partial(
    generic_activation_patch,
    patch_setter = layer_pos_head_vector_patch_setter,
    activation_name = "q",
    index_axis_names = ("layer", "pos", "head")
)
attn_head_k_act_patch = partial(
    generic_activation_patch,
    patch_setter = layer_pos_head_vector_patch_setter,
    activation_name = "k",
    index_axis_names = ("layer", "pos", "head")
...
# %%
attn_head_out_all_pos_act_patch = partial(
    generic_activation_patch,
