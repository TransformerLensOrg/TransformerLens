...
#%% [markdown] what's the copy score?
...
def check_copy_circuit(model, layer, head, ioi_dataset, verbose=False, neg=False):
    ...
    # print(
    #     model.blocks[layer].attn.W_V[head].T.shape,
    #     model.blocks[layer].attn.b_V[head].shape,
    # )
    print(z_0.shape)
    v = torch.einsum(  # TODO fix messed up shapes
        "eab,cb->eac",
        z_0,
        model.blocks[layer].attn.W_V[head] + model.blocks[layer].attn.b_V[head],
    )
