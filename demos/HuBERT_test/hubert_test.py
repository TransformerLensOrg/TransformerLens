...
def run_basic_sanity_tests(model, waveform_np):
    ...
    out_train = model(x)
    out_train_tensor = (
        out_train if isinstance(out_train, torch.Tensor) else out_train["predictions"]
    )
...
def optional_compare_to_hf(your_model, waveform_np, sr=SAMPLE_RATE):
    ...
    input_values = hf_feat(waveform_np, sampling_rate=sr, return_tensors="pt").get(
        "input_values"
    )  # (1, T)
    ...
    your_tensor = (
        your_out if isinstance(your_out, torch.Tensor) else your_out["predictions"]
    )  # shape depends on your model
