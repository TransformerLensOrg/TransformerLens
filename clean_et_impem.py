...
BABA_TEMPLATES = [
    ...
    # #"Then, [B] and [A] went to the [PLACE]. [B] gave a [OBJECT] to [A]",
    # "After"+10*" "+ "[B] and [A] went to the [PLACE]. [B] gave a [OBJECT] to [A]",
    # "After"+11*" "+ "[B] and [A] went to the [PLACE]. [B] gave a [OBJECT] to [A]",
    # "After"+12*" "+ "[B] and [A] went to the [PLACE]. [B] gave a [OBJECT] to [A]",
    # "After"+13*" "+ "[B] and [A] went to the [PLACE]. [B] gave a [OBJECT] to [A]",
    # "After"+14*" "+ "[B] and [A] went to the [PLACE]. [B] gave a [OBJECT] to [A]",
...
)  # ["Two friends were discussing.", "It was a levely day.", "Two friends arrived in a new place.", "The couple arrived."]) # , prompts=saved_prompts) # [{"IO" : "Anthony", "S" : "Aaron", "text" : "Then, Aaron and Anthony went to the grocery store. Aaron gave a ring to Anthony"}, {'IO': 'Lindsey', 'S': 'Joshua', 'text': 'Then, Joshua and Lindsey were working at the grocery store. Joshua decided to give a basketball to Lindsey'}], symmetric=True)
...
owb_seqs = [
    "".join(show_tokens(webtext["train"]["text"][i][:2000], return_list=True)[: ioi_dataset.max_len])
...
K = 1
no_prompts = N
heads_to_keep = get_heads_circuit(ioi_dataset, no_prompts, calib_head=True)
...
old_score = score_metric(model, ioi_dataset[:no_prompts], k=K)
model.reset_hooks()
...
score = score_metric(model, ioi_dataset[:no_prompts], k=K)

# %%
print(f"Logit_diff =  {ldiff.mean()} +/- {std}. Score {score}")
print(f"Original logif_diff =  {old_ld.mean()} +/- {old_std}. Score {old_score}")

df = pd.DataFrame(
    {
        "x": ldiff.cpu(),
        "y": np.random.random(len(ldiff)),
...
def do_global_patching(
    ...
    for layer in mlps.keys():
        ...
        hk, hk_name = ptch.get_hook(layer, head=None, target_module="mlp")
        print(hk)
