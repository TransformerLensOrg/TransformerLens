...
from easy_transformer.utils_circuit_discovery import HypothesisTree
...
def generate_completion(
    model: EasyTransformer, input: str, max_len: int = 3, device=device
):
...
for l1, l2, l3 in itertools.product(uppers, uppers, uppers):
    ...
    if len(model.to_tokens(acro, prepend_bos=False)[0]) == 1:
        ...
        acro[:2] not in two_letter_single_token
        and acro[1:] not in two_letter_single_token
    ):
        all_individual.append(acro)

print(
    f"{len(two_letter_single_token)} two-letter acronyms are their own tokens, which is {len(two_letter_single_token)/(26*26)} of all {26 * 26} possible two-letter acronyms."
)

print(
    f"{len(three_letter_single_token)} three-letter acronyms are their own tokens, which is {len(three_letter_single_token)/(26*26*26)} of all {26*26*26} possible three-letter acronyms."
)

print(
    f"{len(all_individual)} three-letter acronyms don't contain any two-letter substring that are one token, which is {len(all_individual)/(26*26*26)} of all {26*26*26} possible three-letter acronyms."
)
...
def get_phrase_with_acro(
    acro: str, words1: list[str], words2: list[str], words3: list[str]
):
    A, B, C = acro
...
def make_dataset(
    ...
    for i in tqdm(range(n)):
        ...
        while (
            phrase2 is None
        ):  # to make sure we don't get invalid acros (single-token ones)
            ...
            if mode == "12":
                if (
                    acro_len == 2
                    and len(model.to_tokens(acro[1:], prepend_bos=False)[0]) == 1
                ):
                    acro2 = acro
            if mode == "21":
                if (
                    acro_len == 2
                    and len(model.to_tokens(acro[:2], prepend_bos=False)[0]) == 1
                ):
                    acro2 = acro
            if acro2 is None:
                continue
        ...
        if mode == "first" or mode == "second" or mode == "third":
            ...
            dataset.append(
                (
                    {"full_prompt": prompt, "phrase": phrase, "acro": acro},
                    {"full_prompt": prompt2, "phrase": phrase2, "acro": acro2},
                )
            )
    return dataset
...
class FakeOrgsDataset:
    def __init__(
        self, N: int, dataset: list[dict] = None, tokenizer=None, mode="default"
    ):
        self.N = N
        ...
        if mode == "first" or mode == "second" or mode == "third":
            self.text_prompts = (
                [data[0]["full_prompt"] for data in self.dataset],
                [data[1]["full_prompt"] for data in self.dataset],
            )
            self.toks = (
                model.to_tokens([data[0]["full_prompt"] for data in self.dataset]),
            ...
            self.acros = (
                [data[0]["acro"] for data in self.dataset],
                [data[1]["acro"] for data in self.dataset],
            )
            self.acro_pos = (
                [get_acro_pos(prompt) for prompt in self.text_prompts[0]],
                [get_acro_pos(prompt) for prompt in self.text_prompts[1]],
            )
            self.max_len = (
                max(
                    [
                        len(self.tokenizer(prompt[0]).input_ids)
                        for prompt in self.text_prompts
                    ]
                )
                + 1
...
def acro_log_probs(
    model: EasyTransformer, text_prompts: list[str], dataset: FakeOrgsDataset
):
