...
class ActivationCache:
    ...
    def __init__(self, cache_dict: dict, model, has_batch_dim: bool = True):
    ...
    def stack_head_results(
        ...
        if components:
            ...
            components = einops.rearrange(
                components, "... head_index d_model -> head_index ... d_model"
            )
            if incl_remainder:
    ...
    def stack_neuron_results(
        ...
        if components:
            components = torch.cat(components, dim=-2)
            components = einops.rearrange(
                components, "... neuron_index d_model -> neuron_index ... d_model"
            )
