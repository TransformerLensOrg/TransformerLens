...
def convert_gpt2_weights(gpt2, cfg: EasyTransformerConfig):
    ...
    state_dict["embed.W_E"] = gpt2.transformer.wte.weight.T
    state_dict["pos_embed.W_pos"] = gpt2.transformer.wpe.weight.T
    ...
    for l in range(cfg.n_layers):
        ...
        W_Q = einops.rearrange(W_Q, "m (i h)->i h m", i=cfg.n_heads)
        W_K = einops.rearrange(W_K, "m (i h)->i h m", i=cfg.n_heads)
        W_V = einops.rearrange(W_V, "m (i h)->i h m", i=cfg.n_heads)
        ...
        W_O = einops.rearrange(W_O, "(i h) m->i m h", i=cfg.n_heads)
    ...
    state_dict[f"unembed.W_U"] = gpt2.lm_head.weight
    state_dict[f"unembed.b_U"] = torch.zeros(cfg.d_vocab)
...
def convert_neo_weights(neo, cfg):
    ...
    state_dict["pos_embed.W_pos"] = neo.transformer.wpe.weight.T
    ...
    for l in range(cfg.n_layers):
        ...
        W_Q = einops.rearrange(W_Q, "(i h) m->i h m", i=cfg.n_heads)
        W_K = einops.rearrange(W_K, "(i h) m->i h m", i=cfg.n_heads)
        W_V = einops.rearrange(W_V, "(i h) m->i h m", i=cfg.n_heads)
        state_dict[f"blocks.{l}.attn.W_Q"] = W_Q
        state_dict[f"blocks.{l}.attn.W_K"] = W_K
        state_dict[f"blocks.{l}.attn.W_V"] = W_V
        ...
        W_O = einops.rearrange(W_O, "m (i h)->i m h", i=cfg.n_heads)
        state_dict[f"blocks.{l}.attn.W_O"] = W_O
        ...
        state_dict[f"blocks.{l}.mlp.W_in"] = neo.transformer.h[l].mlp.c_fc.weight
        state_dict[f"blocks.{l}.mlp.b_in"] = neo.transformer.h[l].mlp.c_fc.bias
        
        state_dict[f"blocks.{l}.mlp.W_out"] = neo.transformer.h[l].mlp.c_proj.weight
    ...
    state_dict["unembed.W_U"] = neo.lm_head.weight
...
def convert_opt_weights(opt, cfg):
    ...
    state_dict["pos_embed.W_pos"] = opt.model.decoder.embed_positions.weight.T[:, 2:]
    ...
    for l in range(cfg.n_layers):
        ...
        state_dict[f"blocks.{l}.mlp.W_in"] = opt.model.decoder.layers[l].fc1.weight
        state_dict[f"blocks.{l}.mlp.W_out"] = opt.model.decoder.layers[l].fc2.weight
    ...
    state_dict[f"final_ln.w"] = opt.model.decoder.final_layer_norm.weight
    state_dict[f"final_ln.b"] = opt.model.decoder.final_layer_norm.bias
    state_dict["unembed.W_U"] = opt.lm_head.weight
    state_dict["unembed.b_U"] = torch.zeros(cfg.d_vocab)
    return state_dict
