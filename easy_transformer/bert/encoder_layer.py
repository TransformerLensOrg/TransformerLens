...
class SelfAttention(nn.Module):
    ...
    def attention_pattern(self, x: TT["batch", "seq", "hidden"]) -> TT["batch", "head", "seq", "seq"]:
        # TODO double check that this is correct
        q = self.w_q(x)
        k = self.w_k(x)
        q = einops.rearrange(q, "batch seq (head head_size) -> batch head seq head_size", head=self.config.n_heads)
        k = einops.rearrange(k, "batch seq (head head_size) -> batch head seq head_size", head=self.config.n_heads)
        result = einsum("batch head seq_q head_size, batch head seq_k head_size -> batch head seq_q seq_k", q, k)
        return result / (self.config.hidden_size ** 0.5)

    def forward(self, x: TT["batch", "seq", "hidden"], mask = None) -> TT["batch", "seq", "hidden"]:
        # here we do the computation per head
        attention = self.attention_pattern(x) if mask is None else self.attention_pattern(x) + mask
        attention = attention.softmax(dim = -1)
        v = self.w_v(x)
        v = einops.rearrange(v, "b seq (head head_size) -> b head seq head_size", head=self.config.n_heads)
        ...
        rearranged = einops.rearrange(combined_values, "b head seq head_size -> b seq (head head_size)"))
